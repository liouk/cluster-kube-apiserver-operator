...
const (
	...
	SourceAuthConfigCMNamespace = "openshift-config"
...
func (o *externalOIDC) ObserveExternalOIDC(genericListers configobserver.Listers, recorder events.Recorder, existingConfig map[string]interface{}) (map[string]interface{}, []error) {
	...
	authConfigCM, authConfigErr := listers.ConfigMapLister().ConfigMaps(SourceAuthConfigCMNamespace).Get(AuthConfigCMName)

	sourceName := ""
	sourceNamespace := ""
	observedConfig := make(map[string]interface{})
	if auth.Spec.Type == configv1.AuthenticationTypeOIDC {
		sourceName = AuthConfigCMName
		sourceNamespace = SourceAuthConfigCMNamespace

		if authConfigErr != nil {
			return existingConfig, append(errs, fmt.Errorf("failed to get configap %s/%s: %v", SourceAuthConfigCMNamespace, AuthConfigCMName, authConfigErr))
		}

		if validationErr := validateAuthConfigMap(authConfigCM); validationErr != nil {
			return existingConfig, append(errs, fmt.Errorf("configmap %s/%s is invalid: %v", SourceAuthConfigCMNamespace, AuthConfigCMName, validationErr))
		}

		if err := unstructured.SetNestedField(observedConfig, staticAuthConfigPath, apiServerArgumentsPath, argAuthConfig); err != nil {
			return existingConfig, append(errs, err)
		}
	} else if authConfigCM == nil {
		// auth type is not OIDC and no auth config exists; nothing to do
